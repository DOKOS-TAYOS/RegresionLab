...
class TestGenerateTrigonometricFunction:
    ...
    def test_trigonometric_functions(
        self, func_type: str, test_input: tuple, expected: float
    ) -> None:
...
class TestGenerateInverseFunction:
    ...
    def test_inverse_power(
        self, power: int, x_val: float, param: float, expected: float
    ) -> None:
...
class TestPredefinedFunctions:
    ...
    def test_function_evaluation(
        self, func: Callable[..., float], args: tuple, expected: float
    ) -> None:
...
class TestFunctionsWithArrays:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                sin_function,
                (np.array([0.0, np.pi / 2, np.pi]), 1.0, 1.0),
                np.array([0.0, 1.0, 0.0]),
            ),
            (
                ln_function,
                (np.array([1.0, np.e, np.e**2]), 1.0),
                np.array([0.0, 1.0, 2.0]),
            ),
        ],
    )
    def test_array_inputs(
        self, func: Callable[..., NDArray], args: tuple, expected: NDArray
    ) -> None:
