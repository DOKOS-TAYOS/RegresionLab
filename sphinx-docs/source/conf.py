...
def _patch_link_translator() -> None:
    ...
    def _patched_visit(
        self: "relativeinclude.LinkTranslator", node: nodes.Node
    ) -> None:
...
def _skip_imported_member(
    app, what: str, name: str, obj: object, skip: bool, options: dict
) -> bool:
...
def _convert_literal_markdown_paragraphs(
    app, doctree: nodes.document, docname: str
) -> None:
    ...
    for node in list(doctree.traverse(nodes.paragraph)) + list(
        ...
        if match:
            ...
            raw = nodes.raw(
                "", f'<a href="{escaped_uri}">{escaped_text}</a>', format="html"
            )
            node.replace_self(raw)
            continue
        ...
        if match_img and _IMAGE_INLINE_RE.search(text, match_img.end()) is None:
            ...
            new_children.append(
                nodes.raw(
                    "",
                    f'<img src="{escape(uri)}" alt="{escape(alt)}" />',
                    format="html",
                )
            )
            if after:
...
def _rewrite_doc_md_links(app, doctree, docname):
    ...
    def refuri_for(base: str) -> str:
        ...
        if not html or not get_uri:
            return html or ""
        target_doc = _DOC_HTML_TO_DOCNAME.get(html) or (
            base.replace(".html", "") if base.endswith(".html") else None
        )
        if target_doc:
    ...
    for node in doctree.traverse(nodes.reference):
        for attr in ("refuri", "refid"):
            ...
            if not raw:
                continue
            base = _norm_base(raw)
            if base in _DOC_MD_TO_HTML or (
                base.endswith(".html")
                and base.replace(".html", "")
                in {v.replace(".html", "") for v in _DOC_MD_TO_HTML.values()}
            ):
    ...
    if pending_xref is not None:
        doc_to_html = {
            **_DOC_MD_TO_HTML,
            ...
            "streamlit-guide": "streamlit-guide.html",
            "tkinter-guide": "tkinter-guide.html",
        ...
        for node in list(doctree.traverse(pending_xref)):
            ...
            base = (
                _norm_base(reftarget) if reftarget else (str(reftarget).strip() or "")
            )
            if base in doc_to_html:
                html = doc_to_html[base]
                target_doc = html.replace(".html", "")
                uri = (
                    get_uri(docname, target_doc) if get_uri else html
                ) + _get_fragment(reftarget)
                ref = nodes.reference("", "", *node.children, refuri=uri, internal=True)
...
def _on_missing_reference(app, env, node, contnode):
    ...
    try:
        ...
        uri = html + fragment
    ref = nodes.reference("", "", *contnode.children, refuri=uri, internal=True)
...
html_static_path = (
    ["_static", _images_src] if os.path.isdir(_images_src) else ["_static"]
)
...
man_pages = [(root_doc, "regressionlab", "RegressionLab Documentation", [author], 1)]
...
texinfo_documents = [
    (
        ...
        author,
        "RegressionLab",
        "Scientific curve fitting application with GUI",
        "Miscellaneous",
    ),
]
