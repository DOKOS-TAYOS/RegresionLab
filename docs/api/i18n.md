...
- **Spanish** (`'es'` or `'español'`): Default language
- **English** (`'en'` or `'english'`): Secondary language
...
# Returns: 'Bienvenido, científico. ¿Qué deseas hacer?' (in Spanish)

# Translation with formatting
error_msg = t('error.fitting_failed_details', error='Invalid data')
# Returns: 'RegressionLab no ha sido capaz de ajustar los datos.\n\nDetalles: Invalid data'
```
...
# Result: "Datos cargados: 100 filas, 5 columnas"
```
