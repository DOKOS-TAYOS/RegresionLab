        ...
        if fit_info is not None:
            ...
            if fit_func is not None and params is not None:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error evaluating fit at linspace: {e}. Using x, y_fitted."
                    )
...
def create_3d_plot(
    ...
    try:
        ...
        if fit_info is not None:
            ...
            if fit_func is not None and params is not None:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error evaluating fit on 3D grid: {e}. Using interpolation."
                    )
                    Z_mesh = None
        if Z_mesh is None:
            ...
            if griddata is not None:
                try:
                    ...
                    logger.debug(
                        "Linear interpolation failed (%s), using nearest neighbor",
                        type(e).__name__,
                    )
                    Z_mesh = griddata(
                        (x_arr, y_arr),
            ...
            if Z_mesh is None:
                ...
                for start in range(0, n_grid, chunk_size):
                    ...
                    dists_sq = (X_flat[start:end, np.newaxis] - x_arr) ** 2 + (
                        Y_flat[start:end, np.newaxis] - y_arr
                    ) ** 2
        ...
        if interactive:
            logger.info(
                "3D plot prepared: %s (interactive, will save on close)", save_path
            )
