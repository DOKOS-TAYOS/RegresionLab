...
def csv_reader(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(
            t("log.successfully_loaded_csv", rows=len(data), columns=len(data.columns))
        )
        return data
...
def txt_reader(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        validate_data_format(data)
        logger.info(
            t("log.successfully_loaded_txt", rows=len(data), columns=len(data.columns))
        )
        return data
    except pd.errors.EmptyDataError:
        logger.error(t("log.csv_file_empty", path=file_path))
...
def excel_reader(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(
            t("log.error_reading_excel", path=file_path, error=str(e)), exc_info=True
        )
