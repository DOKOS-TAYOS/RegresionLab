...
def validate_file_type(
    ...
    """
    effective_types = (
        allowed_types if allowed_types is not None else list(DATA_FILE_TYPES)
    )
...
def validate_dataframe(data: pd.DataFrame, min_rows: int = 2) -> None:
    ...
    logger.debug(
        t("log.dataframe_validated", rows=len(data), columns=len(data.columns))
    )
...
def _validate_numeric_data(data: pd.Series, column_name: str) -> None:
    ...
    if not pd.api.types.is_numeric_dtype(data):
        logger.error(
            t("log.column_not_numeric", column=column_name, dtype=str(data.dtype))
        )
        raise DataValidationError(t("error.column_must_be_numeric", column=column_name))

    nan_count = data.isna().sum()
    if nan_count > 0:
        logger.warning(
            t("log.column_contains_nan", column=column_name, count=nan_count)
        )
        raise DataValidationError(
            t(
                "error.column_contains_missing_values",
                column=column_name,
                count=nan_count,
            )
        )
    ...
    if inf_mask.any():
        logger.error(
            t("log.column_contains_infinite", column=column_name, count=inf_mask.sum())
        )
        raise DataValidationError(
...
def _validate_uncertainty_column(data: pd.DataFrame, var_name: str) -> None:
    ...
    if neg_mask.any():
        logger.error(
            t("log.uncertainty_negative", column=uncertainty_col, count=neg_mask.sum())
        )
        raise DataValidationError(
...
def validate_parameter_names(param_names: List[str]) -> None:
    ...
    for name in param_names:
        if not name.isidentifier():
            ...
            raise ValidationError(t("error.invalid_parameter_name", name=name))
...
def _validate_no_completely_empty_rows(data: pd.DataFrame) -> None:
    ...
    if empty_rows.any():
        ...
        logger.warning(
            t(
                "log.empty_rows_found",
                count=empty_count,
                rows=str(empty_row_indices[:10]),
            )
        )
...
def validate_data_format(data: pd.DataFrame) -> None:
    ...
    logger.info(
        t("log.data_format_validated", rows=len(data), columns=len(data.columns))
    )
